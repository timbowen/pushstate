...
;(function($){
    ...
    EggLoader.prototype.loadContent = function() {
		...
		$.ajax({
            ...
            success: function( response )
            {
				...
				// iterate over the scripts on the ajax page and if the script does not exist on the original page, add it;
				targetScripts.each(function(){
					if( 'string' == typeof this.src ){
						if( -1 == $.inArray(this.src , $originScripts ) ){
							$originScripts.push(this.src);
    ...
    if ( typeof history.pushState !== "undefined" ){   					// history.pushState is undefined for IE8 and lower;
		...
		$('script').each( function(k,v){
			if('string' == typeof v.src ){
				$originScripts.push(v.src);
			}
		});
